...
<div class="user-form">
	...
	<?php else: ?>
		...
		<?=	$form->field($mdoel, 'retypepass')->textInput() ?>
	<?php endif; ?>
    ...
    <?= $form->field($model, 'pict')->widget
	(FileInput::className(),[
		'options'	=> [
			'accept'	=> 'image/*',
			],
			'pluginOptions'	=> [
				'initialPreview'	=>(!$model->isNewRecord ? (!empty($model->pict)? Html::img(\Yii::$app->cv->lihatImageDetail($model->pict, "", $kategori = "user")): NULL) : null),
